...
export default function Contact() {
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-600 text-lg mt-4 font-medium font-body max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-12 w-full max-w-5xl mx-auto text-left font-body justify-center">
        {WHATSAPP_CONTACTS.map((member, index) => {
          ...
          return (
            <div 
              key={member.name} 
              className={`rounded-2xl p-6 bg-white border border-slate-200/60 shadow-sm shadow-slate-100 hover:border-[#25D366]/40 hover:bg-slate-50/30 hover:shadow-md hover:shadow-slate-200/30 transition-all duration-300 flex flex-col justify-between h-full group w-full ${
                isLast ? "sm:col-span-2 md:col-span-1 sm:max-w-[calc(50%-12px)] md:max-w-none sm:mx-auto md:mx-0" : ""
              }`}
            >
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-full flex-shrink-0 flex items-center justify-center bg-gradient-to-tr from-cyan-500 to-violet-600 text-white font-display text-lg font-bold shadow-lg">
                    {member.avatarText}
                  </div>
                  <div>
                    <h4 className="text-slate-900 font-bold font-display group-hover:text-[#25D366] transition-colors">
                      {member.name}
                    </h4>
                  </div>
                </div>
              ...
              <a
                ...
                className="w-full py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-white bg-[#25D366] hover:bg-[#20ba56] hover:scale-[1.02] transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer shadow-md shadow-[#25d366]/20"
              >
