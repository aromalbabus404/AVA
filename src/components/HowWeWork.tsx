...
export default function HowWeWork() {
  return (
    <section 
      id="how-we-work" 
      className="relative py-[30px] md:py-[60px] px-6 w-full overflow-hidden select-none bg-slate-50"
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col gap-4 mb-16 max-w-2xl">
          <span className="text-[12px] font-bold text-cyan-600 uppercase tracking-[2px] font-body">
            OUR PROCESS
          </span>
          <h2 className="text-4xl sm:text-5xl font-bold text-slate-900 font-sans tracking-tight">
            How We Work
          </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full">
          {STEPS.map((step, index) => {
            ...
            return (
              <div 
                key={step.num}
                className={`rounded-2xl p-5 sm:p-8 bg-white border border-slate-200/50 flex flex-col items-start transition-all duration-200 ease-in-out hover:border-cyan-500/30 hover:-translate-y-[2px] hover:bg-slate-50/50 hover:shadow-md hover:shadow-slate-200/40 ${
                  isLast ? "col-span-2 lg:col-span-1" : ""
                }`}
              >
                {/* Step Number Badge */}
                <span className="text-3xl font-extrabold text-cyan-600 font-display mb-4">
                  {step.num}
                </span>
                ...
                {/* Step Description */}
                <p className="text-[14px] leading-[1.6] text-slate-600 font-body">
                  {step.description}
                </p>
              </div>
            );
